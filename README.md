...
Every page then gets navigation, generated ones included: each tag overview page
...
if meta.pageNav && meta.pageNav.elements.length
  nav.page-nav
    each item in meta.pageNav.elements
      a.page-nav__link(href=item.href, class=item.current ? meta.pageNav.activeClass : '') #{ item.name }
```
...
- **Nera**: v4.1.0+ — a baseline rather than a requirement; this plugin uses no
  ...
  needs no pug `basedir`. The **recommended `config/plugin-order.yaml` entry**
  needs **v4.2.0+**, where that file is read; on 4.1.x it is ignored and pages
  generated by other plugins get no navigation
